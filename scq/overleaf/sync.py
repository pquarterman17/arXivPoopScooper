...
def setup_mode(git_url):
    ...
    if result.returncode != 0:
        ...
        print(result.stderr)
        sys.exit(1)
    print(f"  Cloned to .overleaf/repo/")
    ...
    save_config(config)
    print(f"  Config saved to .overleaf/config.json")
    ...
    if ".overleaf/" not in gitignore_text:
        with open(GITIGNORE_PATH, "a") as f:
            f.write("\n.overleaf/\n")
        print(f"  Added .overleaf/ to .gitignore")
    else:
        print(f"  .overleaf/ already in .gitignore")
    ...
    if CONFIG_PATH.exists() and REPO_DIR.exists():
        ...
        print(f"ERROR: Setup incomplete")
        sys.exit(1)

    print(f"\n{'='*60}")
    print(f"SUCCESS — Overleaf sync configured")
...
def sync_mode(force=False):
    ...
    if not config:
        ...
        print(f"Run: python tools/overleaf_sync.py --setup <git-url>")
        sys.exit(1)

    if not REPO_DIR.exists():
        print(f"\nERROR: .overleaf/repo/ not found.")
        print(f"Re-run setup: python tools/overleaf_sync.py --setup <git-url>")
        sys.exit(1)
    ...
    if not has_changes and not force:
        print(f"\n{'='*60}")
        print(f"Already up to date — no changes to push")
        print(f"{'='*60}\n")
        return
    ...
    if result.returncode != 0:
        ...
        print(result.stderr)
        print(f"\nTroubleshooting:")
        print(f"  1. Check your Overleaf Git credentials")
        print(f"  2. Ensure you have push access to the repo")
        print(f"  3. Check if there are uncommitted changes: cd .overleaf/repo && git status")
        sys.exit(1)

    print(f"  Pushed to Overleaf")
...
def main():
    if len(sys.argv) > 1:
        if sys.argv[1] == "--setup":
            ...
            print(f"Usage:")
            print(f"  python tools/overleaf_sync.py                    # sync")
            print(f"  python tools/overleaf_sync.py --setup <git-url>  # setup")
            print(f"  python tools/overleaf_sync.py --status            # status")
            print(f"  python tools/overleaf_sync.py --force              # force sync")
            sys.exit(1)
    else:
        sync_mode()
